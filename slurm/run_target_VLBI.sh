...
apptainer exec -B ${PWD},${BINDPATHS} --no-home ${LOFAR_SINGULARITY} python3 ${FLOCSDIR}/runners/create_ms_list.py LINC target --output_fullres_data --target_skymodel ${DATADIR}/target.skymodel --min_unflagged_fraction 0.1 --get_RM False --cal_solutions ${DATADIR}/LINC-cal_solutions.h5 ${DATADIR} > create_mslist.log
